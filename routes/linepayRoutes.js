...
const linePayClient = createLinePayClient({
  channelId: "2007106620",
  channelSecretKey:"b964536fbb238048114dc7fa29926e93",
...
router.get('/confirm', async (req, res) => {
  ...
  try {
    ...
    console.log('line-pay confirm', linePayResponse)

    // 清除session中的reservation的資料
    if (req.session.reservation) delete req.session.reservation

    res.status(200).json({ ...linePayResponse.body })
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
})
...
router.get('/check-payment-status', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ data: linePayResponse.body })
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
})
